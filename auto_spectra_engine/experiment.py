...
from analysis import plot_PCA, get_plsda_performance

def run_experiment(file, start_index=0, end_index=None, contamination=0.0, combinacao="mc+smo+d1", cortar_extremidades=False, inicio_espectro_col=3, plotar_PCA=False, plotar_plsr=False, plotar_plsda=False, plotar_rf=False, plotar_DDSIMCA=False, plotar_OneClassPLS=False, modelo="PLSDA", inlier_class="Pure", coluna_predicao=0):
    """
    ...
    inicio_espectro = 3 # Qual a coluna que inicia o espectro (valor numerico da coluna)
    plotar_PCA = False # Será gerado os graficos importantes para cada modelo
    plotar_plsr = False
    plotar_plsda = True
    plotar_rf = False
    plotar_DDSIMCA = True
    plotar_OneClassPLS = True

    modelo = "PLSDA" # Qual modelo será avaliado "PLSR", "PLSDA", "RF", "OneClassPLS", "DDSIMCA"
    inlier_class = "Pure" # Caso vá rodar "OneClassPLS", "DDSIMCA" qual o nome da classe alvo?

    coluna_predicao = 0 # Qual a coluna da matriz que será avaliada
    ...
    X = data.iloc[:, inicio_espectro:] # Filtrando o que é a matriz preditora e o que é resposta ou classe
    Ys = data.iloc[:, :inicio_espectro]
    ...
    #print(sub_Ys)
    coluna_y_nome = sub_Ys.columns[coluna_predicao] # Atualiza a coluna_y_nome após a remoção de outliers
    #print(coluna_predicao)
    # Chamando os modelos
    #print(df_pp)
    #print(sub_data)
    #print(sub_Ys)
    ...
    if plotar_PCA:
        plot_PCA(df_pp.values, sub_Ys.iloc[:, coluna_predicao], combinacao)
    ...
    if "PLSR" in modelo:
        ...
        resultados_df.sort_values(['rmse_test']).to_csv(os.path.join(PATH, f"PLSR_Best_{file_name_no_ext}_{coluna_y_nome}.csv"), index=False)  # Save to CSV without index
    ...
    if "PLSDA" in modelo:
        ...
        resultados_df.sort_values(['accuracy'], ascending=False).to_csv(f"{file_name_no_ext}_{coluna_y_nome}_PLSDA_Best_.csv", index=False)  # Save to CSV without index

    if "RF" in modelo:
        # Chamando a função
        accuracy_mean, accuracy_std, accuracy_min, accuracy_max, seed_accuracy_max, sensibility_mean, sensibility_std, sensibility_min, sensibility_max, specificity_mean, specificity_std, specificity_min, specificity_max = get_RF_performance(sub_data.values, sub_Ys.iloc[:, coluna_predicao], test_size=0.33, n_splits=5, plotar_RF=plotar_rf, feature_names = label_espectro)
        ...
        resultados_df.sort_values(['accuracy_mean'], ascending=False).to_csv(os.path.join(PATH, f"RF_Best_{file_name_no_ext}_{coluna_y_nome}.csv"), index=False)  # Save to CSV without index

    if "OneClassPLS" in modelo:
        # Chamando a classe e função
        ocpls_modelo = OneClassPLS(n_components=2, inlier_class=inlier_class, n_splits=3, plotar_OneClassPLS=plotar_OneClassPLS)
        best_accuracy, best_n_components, best_sensitivity, best_specificity = ocpls_modelo.fit_and_evaluate_full_pipeline( sub_data, sub_Ys, coluna_predicao)
        # Armazenamento dos resultados
        ...
        resultados_df.sort_values(['accuracy'], ascending=False).to_csv( os.path.join(PATH, f"best_OneClassPLS_{file_name_no_ext}_{coluna_y_nome}.csv"), index=False) # Save to CSV without index

    if "DDSIMCA" in modelo:
        # Chamando a função
        ddsimca_modelo = DDSIMCA(n_components=2, inlier_class=inlier_class,plotar_DDSIMCA=plotar_DDSIMCA)
        ...
        resultados.append([ file, cortar_extremidades, combinacao, round(contamination, 2), round(best_accuracy, 2), round(best_sensitivity, 2), round(best_specificity, 2), start_index, end_index, round (best_n_components)])
        resultados_df = pd.DataFrame(resultados, columns=[ "file", "cortar_extremidades", "pre_processamento", "contaminacao", "accuracy", "sensitivity", "specificity", "start_index", "end_index", "best_n_components"                          ])
        resultados_df.sort_values(['accuracy'], ascending=False).to_csv( os.path.join(PATH, f"best_DDSIMCA_{file_name_no_ext}_{coluna_y_nome}.csv"), index=False)
